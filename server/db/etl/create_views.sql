...
SELECT
    processop_id,
    ct_id,
    regexp_replace(processop_title, U & '\0097', '-', 'g') AS processop_title,
    cfr_sect,
    ...
    regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(processop_notes, U & '\0097', '-', 'g'), U & '\0085', '...', 'g'), U & '\00A7', '\u00A7', 'g'), U & '\0093', '"', 'g'), U & '\0094', '"', 'g') AS processop_notes,
...
SELECT
    ...
    WHEN pl.lim_id = 52867 THEN
        'Stock Limitations (C16-C18 internal olefin). PAH mass ratio shall not exceed 1 X 10-5. PAH mass ratio = Mass (g) of PAH (as phenanthrene)/Mass (g) of stock base fluid as determined by EPA Method 1654, Revision A, [specified at \u00A7435.11(u)] entitled "PAH Content of Oil by HPLC/UV," December 1992, which is published as an appendix to subpart A of this part and in "Analytic Methods for the Oil and Gas Extraction Point Source Category," EPA-821-R-11-004. See \u00A7435.11(uu).'
    ELSE
        regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(pl.alt_lim, U & '\00A7', '\u00A7', 'g'), U & '\00B0', '\u00B0', 'g'), U & '\0093', '"', 'g'), U & '\0094', '"', 'g'), U & '\00D7', 'X', 'g')
    ...
    pl.mdl,
    pl.ml,
    regexp_replace(pl.lim_calc_desc, U & '\00A7', '\u00A7', 'g') AS lim_calc_desc,
    pl.source_id,
    pl.qc_flag,
    pl.qc_notes,
    pl.zero_discharge,
    ...
    ELSE
        regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(pl.pollutant_notes, U & '\00A7', '\u00A7', 'g'), chr(147), '"', 'g'), chr(148), '"', 'g'), 'ï¿½', 'u', 'g'), U & '\00A0', ' ', 'g'), U & '\00B5', '\u00B5', 'g'), U & '\00D7', 'X', 'g')
...
SELECT
    source_id,
    psc,
    regexp_replace(cfr_section, U & '\00A0', '', 'g') AS cfr_section,
    regexp_replace(regexp_replace(subcategory, U & '\0096', '-', 'g'), U & '\0097', '-', 'g') AS subcategory,
    regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(cfr_section_description, U & '\0093', '"', 'g'), U & '\0094', '"', 'g'), U & '\0096', '-', 'g'), U & '\0097', '-', 'g'), U & '\00A0', '', 'g') AS cfr_section_description,
    publication_date,
    regexp_replace(frn__in_cfr_, U & '\00A0', '', 'g') AS frn__in_cfr_,
    frn__1st_page_,
...
SELECT
    ...
    source_pub_date,
    source_eff_date,
    regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(source_summary, U & '\00A7', '\u00A7', 'g'), chr(145), '''', 'g'), chr(146), '''', 'g'), chr(147), '"', 'g'), chr(148), '"', 'g'), chr(150), '-', 'g'), chr(151), '-', 'g') AS source_summary,
    ...
    regexp_replace(regexp_replace(regexp_replace(display_title, chr(150), '-', 'g'), chr(151), '-', 'g'), U & '\00A0', '', 'g') AS display_title,
    source_notes,
    fr_revision_type,
...
SELECT
    ...
    treatment_id,
    tech_ref,
    regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(tech_notes, chr(147), '"', 'g'), chr(148), '"', 'g'), U & '\0085', '...', 'g'), chr(150), '-', 'g'), chr(151), '-', 'g'), U & '\00A7', '\u00A7', 'g') AS tech_notes,
    bmp_type,
    CASE WHEN zero_discharge = '1' THEN
        TRUE
    ELSE
