...
module.exports = {
  up: (queryInterface, Sequelize) =>
    queryInterface
      ...
      .then(() => {
        queryInterface.addIndex({ schema: 'elg_search', tableName: 'Limitation' }, ['processop_id']).then(() => {
          queryInterface.addIndex({ schema: 'elg_search', tableName: 'Limitation' }, ['pollutant_code']).then(() => {
            queryInterface.addIndex({ schema: 'elg_search', tableName: 'Limitation' }, ['alt_lim_flag']).then(() => {
              queryInterface.addIndex({ schema: 'elg_search', tableName: 'Limitation' }, ['alt_lim']).then(() => {
                queryInterface.addIndex({ schema: 'elg_search', tableName: 'Limitation' }, ['pollutant_notes']);
              });
            });
          });
        });
