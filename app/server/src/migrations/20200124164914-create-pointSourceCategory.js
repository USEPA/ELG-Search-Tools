  ...
  up: (queryInterface, Sequelize) =>
    queryInterface.sequelize.query('drop schema if exists elg_search cascade; create schema elg_search;').then(() => {
      queryInterface
        ...
        .then(() => {
          queryInterface
            .addIndex({ schema: 'elg_search', tableName: 'PointSourceCategory' }, ['IncludeInSearchTool'])
            .then(() => {
              queryInterface.addIndex({ schema: 'elg_search', tableName: 'PointSourceCategory' }, ['psc_name']);
            });
        });
    }),
