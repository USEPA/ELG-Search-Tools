...
function parsePollutantId(pollutantId) {
  ...
  try {
    result = pollutantId
      ? decodeURIComponent(pollutantId)
          .split(';')
          .map((id) => id.split('|'))
          .flat()
      : [];
...
function parseMultiCriteriaSearchParams(query) {
  ...
  let filterPointSourceCategoryCodes = query.filterPointSourceCategoryCode
    ? query.filterPointSourceCategoryCode.split(';')
    : [];
...
function validateMultiCriteriaSearchParams(params) {
  return new Promise((resolve, reject) => {
    try {
      ...
      let queryColumns = limitation.attributes
        .concat(limitation.downloadAttributes)
        .concat([
          'treatmentId',
          'treatmentCodes',
          'treatmentNames',
          ...
          .then((limitationUnit) => {
            if (params.rangeUnitCode && limitationUnit === null) {
              ...
              params.treatmentTechnologyGroups.forEach((treatmentTechnologyGroup) => {
                treatmentTechnologyGroupsValid =
                  treatmentTechnologyGroupsValid &&
                  [
              ...
              if (treatmentTechnologyGroupsValid) {
                PollutantGroup.findAll({
                  ...
                  .catch(() => reject('Invalid value passed for pollutantGroupId'));
              } else {
                reject('Invalid value passed for treatmentTechnologyGroup');
              }
            }
          })
          .catch(() => reject('Invalid value passed for rangeUnitCode'));
      }
    ...
    return PointSourceCategory.findAll({
      ...
      .then((pointSourceCategories) => {
        SicCode.findAll({
          attributes: [
            'sicCode',
            [Sequelize.literal("lpad(sic_code::text, 4, '0')"), 'sicCodeDisplay'],
            'sicDescription',
          ],
          where: {
            ...
            NaicsCode.findAll({
              ...
              where: {
                ...
                [Op.and]: Sequelize.literal(
                  'naics_code in (select naics_code from elg_search."PointSourceCategoryNaicsCode")'
                ),
              },
              ...
              .then((naicsCodes) => {
                Pollutant.findAll({
                  attributes: [
                    ...
                    [
                      Sequelize.literal("string_agg(distinct pollutant_desc, '|' order by pollutant_desc)"),
                      'pollutantId',
                    ],
                  ],
                  where: {
                    [Op.and]: [
                      Sequelize.literal(
                        'pollutant_code in (SELECT DISTINCT pollutant_code FROM elg_search."ViewLimitation")'
                      ),
                    ],
                  },
                      ...
                      .then((pollutantGroups) => {
                        TreatmentTechnologyCode.findAll({
                          ...
                          where: {
                            [Op.and]: [
                              Sequelize.literal(
                                'code in (select regexp_split_to_table(treatment_codes, \'; \') from elg_search."ViewWastestreamProcessTreatmentTechnology")'
                              ),
                            ],
                          },
                          order: ['name'],
  ...
  multiCriteriaSearch(req, res) {
    ...
    validateMultiCriteriaSearchParams(params)
      .then((validParams) => {
        ...
        criteriaDisplayPromises.push(
          PointSourceCategory.findAll({
            where: { pointSourceCategoryCode: { [Op.in]: pointSourceCategoryCodes } },
            order: ['pointSourceCategoryCode'],
          }).then((codes) => {
            pointSourceCategoryDisplay = codes
              .map((a) => a.pointSourceCategoryCode + ': ' + a.pointSourceCategoryName)
              .join(', ');
          })
        );
        ...
        criteriaDisplayPromises.push(
          SicCode.findAll({
            ...
            sicCodeDisplay = codes.map((a) => a.sicCodeAsNumber + ': ' + a.sicDescription).join(', ');
          })
        );
        ...
        criteriaDisplayPromises.push(
          NaicsCode.findAll({
            ...
            naicsCodeDisplay = codes.map((a) => a.naicsCodeAsNumber + ': ' + a.naicsDescription).join(', ');
          })
        );
        ...
        criteriaDisplayPromises.push(
          Pollutant.findAll({
            ...
            pollutantDisplay = codes.map((a) => a.elgDescription).join(', ');
          })
        );
        ...
        criteriaDisplayPromises.push(
          PollutantGroup.findAll({
            ...
            pollutantGroupDisplay = codes.map((a) => a.description).join(', ');
          })
        );
        ...
        criteriaDisplayPromises.push(
          TreatmentTechnologyCode.findAll({
            ...
            treatmentTechnologyDisplay = codes.map((a) => a.name).join(', ');
          })
        );
          ...
          .then(() => {
            if (downloadRequested) {
              limitation
                ...
                .then((dataStream) => {
                  download.createDownloadFileFromStream(
                    'limitations',
                    'Limitations',
                ...
                .catch((error) =>
                  res.status(400).send('Error getting data for download: ' + utilities.sanitizeError(error))
                );
            } else {
                  ...
                  PointSourceCategory.findAll({
                    ...
                    where: {
                      pointSourceCategoryCode: {
                        [Op.in]: [...new Set(allLimitations.rows.map((a) => a.pointSourceCategoryCode))],
                      },
                    },
                    order: ['pointSourceCategoryCode'],
                  })
                    .then((pscs) => {
                      Pollutant.findAll({
                        ...
                        where: {
                          elgDescription: {
                            [Op.in]: [...new Set(allLimitations.rows.map((a) => a.pollutantDescription))],
                          },
                        },
                        group: ['elg_pollutant_description'],
    ...
    validateSortParams(req.query.sortCol, req.query.sortDir, req.query.offset, req.query.limit)
      .then(() => {
        if (!['AND', 'OR'].includes(operator)) {
          ...
          if (downloadRequested) {
            limitation
              .keywordSearchLimitationsForDownload(keywords, operator, sortCol, sortDir)
              .then((dataStream) => {
                download.createDownloadFileFromStream(
                  'limitations',
                  'Limitations',
                  ...
                  [
                    {
                      label: 'Keyword(s)',
                      value: keywords.map((a) => a.replace(/%/g, '')).join(' ' + operator + ' '),
                    },
                  ],
                  dataStream,
                  res
            ...
            limitation
              .keywordSearchLimitations(keywords, operator, sortCol, sortDir, offset, limit)
              .then((result) => {
                Pollutant.findAll({
                  attributes: [
                    ['elg_pollutant_description', 'pollutantDescription'],
